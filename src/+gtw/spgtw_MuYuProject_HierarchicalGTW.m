function [spLst,cx,tDly,neiLst,sv_spLabels,rgt00,isFail,riseDur,dFAvg] = spgtw_MuYuProject_HierarchicalGTW(...
...
clear dFVec; clear dF; clear seMap0;
...
% opts.maxStp = 10;
% tic;
% [paths] = hierarchicalGTW(ref, tst, s, t, smoBases, maxShift,opts.maxStp, stds);


T00 = size(tst,2);
distMatrix = zeros(T00,T00,size(tst,1));
for i = 1:size(tst,1)
...
clear distMatrix;
midPoints = BILCO(ref,tst,[s,t],smoBases(1),initialCut,maxShift);

paths = cell(size(tst,1),1);
for i = 1:size(midPoints,1)
   paths{i} = midPoint2path(midPoints(i,:),T00,T00);
end
% toc;
%% warp curves
cx = gtw.warpRef2Tst_alter(paths,refBase/max(refBase(:)),[H,W,numel(refBase)]);
...
% end
 % [svLabel] = burst.riseMap2evtMuYu_Alter3(spLst,tDly,neiLst,sv_spLabels,10,10,[H,W],spSz,riseDur);
end
function [L] = spConn(L)
	[H,W] = size(L);
    spLst00 = label2idx(L);
