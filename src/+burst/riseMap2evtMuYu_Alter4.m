function [svLabel] = riseMap2evtMuYu_Alter4(spLst,tDly,neibLst,sv_spLabels,maxRiseUnc,cDelay,sz,spSz,riseDur,dFAvg)
...
%% get histogram of meanIntensity of super pixels
avgIntensity = zeros(nSp,1);
for i = 1:nSp
    avgIntensity(i) = mean(dFAvg(spLst{i}));
end
weakSp = find(avgIntensity<0.4*max(avgIntensity));
% seedLabel = setdiff(seedLabel,weakSp);
tDly(weakSp) = tDly(weakSp)+100;
