function updtFeatureRegionLandmarkNetworkShow(f,datR,evtLst,ftsLst,gg,nCh)
...
try
    if ~isempty(regLst)
        ...
        %ftsLst = [];  % !!DBG
        regMask = ones(numel(ftsLst.loc.x3D),1);
    end
    if(nCh == 1)
...
if(nCh==1)
    setappdata(f,'fts1',ftsLst);
else
    setappdata(f,'fts2',ftsLst);
end
