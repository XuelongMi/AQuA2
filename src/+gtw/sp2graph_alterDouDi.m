function [ref,tst,refBase,s,t,idxGood,orgCurves,stds] = sp2graph_alterDouDi(rDF,df0ip,m0Msk,rgt00,vMap0,spLst,seedIn,gapSeedHW,majorInfo,sv_spLabels)
...
refBase = curveSmo;
%%
peakTW = ts:min(tPeak+ext,T0);
refBase(1:ts) = refBase(ts);
[~,tPeak] = max(curveSmo(peakTW));
tPeak = ts + tPeak - 1;
refBase(tPeak:end) = refBase(max(peakTW));
