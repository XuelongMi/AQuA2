function [spLst,cx,tDly,neiLst,sv_spLabels,rgt00,isFail] = spgtw_MuYuProject_Alter3(...
...
[ref,tst,refBase,s,t,idxGood,orgCurves,s2] = gtw.sp2graph_alter5(dat,dF,m0Msk,rgt00,validMap,spLst,seedIn,gapSeed,majorInfo,sv_spLabels);
% gtw
% spLst = spLst(idxGood);
% nSp = numel(spLst);
...
function spLst = mergeSmallSp(L,spSzMinLimitation,mtPeakTW,mTW,dFVec,dif)
    ...
    while(1)
        ...
        for i = 1:numel(checkRegions)
            ...
            noSignal = curDif<min(dif/3,3*s0);
