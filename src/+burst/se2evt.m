...
spSz = 200;  % preferred super pixel size
spT = 30;  % super pixel number scale (larger for more)

% GTW on super pixels
% group super pixels to events
if numel(ihw0)>30
    [spLst,cx,dlyMap,distMat,rgtSel,xFail,~,~] = gtw.spgtw_MuYuProject(...
        dF0,seMap0,seSel,gtwSmo,maxStp,cDelay,spSz,spT,opts);
    if xFail==0
        % smooth propagation first
        [evtMap] = burst.riseMap2evtMuYu(spLst,dlyMap,distMat,maxRiseUnc,cDelay);
        % smooth propagation first
...
rgttt = min(rgtx)-min(rgt)+1:(numel(rgt)-max(rgt)+max(rgtx));
[evtL,evtRecon] = gtw.evtRecon_MuYuProject(spLst,cx1,evtMap,minShow,seMap0(:,:,rgttt)==seSel);
evtRecon = evtRecon.^2;
